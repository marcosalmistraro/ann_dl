...
% After training the second autoencoder, you train the third autoencoder in
% a similar way. You decrease the size of the hidden representation to
% 30, so that the encoder in the third autoencoder learns an even smaller
% representation of the input data.
...
% passing them through the first encoder, this was reduced to 100
% dimensions. After using the second encoder, this was reduced again to 50
% dimensions. Then again, passing the data through the third encoder 
% reduced the amount of features to 30. You can now train a final layer to 
% classify these 30-dimensional vectors into different digit classes.


%press any key to go on
pause;
%% Training the final softmax layer
% Train a softmax layer to classify the 30-dimensional feature vectors.
% Unlike the autoencoders, you train the softmax layer in a supervised
% fashion using labels for the training data.

softnet = trainSoftmaxLayer(feat3,tTrain,'MaxEpochs',80);
...
% networks that you have trained. 
% They are |autoenc1|, |autoenc2|, |autoenc3| and |softnet|.
...
pause;
%%
% As it was explained, the encoders from the autoencoders have been used to
