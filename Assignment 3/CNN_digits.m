...
%%
% *Image Input Layer* An input layer is where
...
%%
% *Convolutional Layer*
% In the convolutional layer, the first argument is |filterSize|, which is 
% the height and width of the filters the training function uses while
...
% call to convolution2dLayer.

%% 
% *ReLU Layer*
% The convolutional layer is followed by a nonlinear activation function.
% MATLAB uses the rectified linear unit function, specified by reluLayer
...
% |poolSize|. In this example, the size of the rectangular region is [2,2].
% The optional argument |Stride| determines the step size the training 
% function takes as it scans along the image. This max-pooling layer 
% takes place between the convolutional layers when there are multiple 
...
% using the softmaxLayer function after the last fully connected layer.

%%
% *Classification Layer* 
% The final layer is the classification layer, defined by using the
% classificationLayer function. This layer uses
...
% is a full training cycle onthe whole training data), and start the
...
% between. The mini-batch loss is calculated as cross-entropy loss> 
% and the mini-batch accuracy is the percentage of images 
% in the current mini-batch that the network being trained 
