...
for p_index=1:length(pArray)
    [trainingX,trainingY]=getTimeSeriesTrainData(lasertrain, ...
        pArray(p_index));
    [testX,testY]=getTimeSeriesTrainData([lasertrain;laserpred], ...
        pArray(p_index));
    ...
    for i=120:120
        ...
        [nets{p_index}{i},tr{p_index}{i}]=train(nets{p_index}{i}, ...
            trainingP,trainingT);
        simulationTraining{p_index}{i}=sim(nets{p_index}{i},trainingP);
        mseTraining{p_index}{i} = mean((trainingY ...
            - cell2mat(simulationTraining{p_index}{i})).^2);
        ...
        for j=1:length(testY)
            ...
            mseValidation{p_index}{i} = mseValidation{p_index}{i} + ( ...
                lastOutput-testY(j))^2;
        end
        ...
        for repetition=2:10
            ...
            for j=1:length(testY)
                ...
                tempMseValidation = tempMseValidation + ( ...
                    lastOutput-testY(j))^2;
            end
...
for p_index=1:length(pArray)
    ...
    step = max(1,round(maxHidden / 5));
    plot3(p_index*(ones(length(cell2mat(mseTraining{p_index})), 1)), ...
        1:step:maxHidden+1,cell2mat(mseTraining{p_index}), ...
        'DisplayName','Training');
    hold on;
    plot3(p_index*(ones(length(cell2mat(mseTraining{p_index})), 1)), ...
        1:step:maxHidden+1,cell2mat(mseValidation{p_index}), ...
